...
class MediaGenerationPipeline:
    ...
    def prepare_candidate(self, production, project_id: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        # Calling bridges may encounter an already compiled request.
        ...
        anchors: list[str] = []
        parents = [source_id, *[pid for pid in target.get("parent_asset_ids", [])
        ...
        for asset in profiles:
            if asset.get("asset_role") == "character_appearance" and asset.get("status") == "ready" and asset.get("dependency_state") != "stale":
                ...
                appearance_owners.update(eid for eid in asset.get("entity_ids") or []
        ...
        for asset in profiles:
            ...
            if asset["asset_role"] == "character_profile" and matched & appearance_owners:
                try:
                    ...
                    profile = {}
                raw = json.dumps({"stable_profile": profile.get("stable_profile", {})}, ensure_ascii=False)
            anchors.append(raw)
        if context.get("asset_identity_type") == "character" and not selections:
            selections = [{"character_id": eid, "appearance_version": context["appearance_version"] or "v1"}
                          for eid in entity_ids]
        contract = GenerationContract(
            asset_id=target["asset_id"], asset_version=str(target["version"]), prompt=description,
            visual_direction=direction, visual_context=context, entity_ids=entity_ids,
            character_appearances=tuple(selections), identity_anchors="\n".join(anchors),
        )
        reference = target.get("asset_role") in {
            "character_reference", "character_turnaround", "location_reference", "scene_reference", "prop_reference",
        }
        params = dict(payload.get("params") or {})
        compiled = self.prompt_compiler.compile(
            asset_kind=context.get("asset_identity_type", ""), asset_description=description,
            visual_direction=VisualDirection(**{f.name: direction[f.name] for f in fields(VisualDirection) if f.name in direction}),
            contract=contract, reference=reference, provider_negative=str(params.get("negative_prompt") or ""),
        )
        contract_asset = production.create_text_asset(
            project_id, stage=target["stage"], skill="v3-prompt-compiler",
            logical_key=f"{target['logical_key']}:generation-contract", asset_role="generation_contract",
            name="视觉生成合同", content=json.dumps({**asdict(contract), **asdict(compiled)}, ensure_ascii=False, sort_keys=True),
            asset_type="STRUCTURED_DATA", extension=".json", entity_ids=list(entity_ids),
            parent_asset_ids=list(dict.fromkeys(parents)), metadata={"visual_context": context},
        )
        prompt_asset = production.create_text_asset(
            project_id, stage=target["stage"], skill="v3-prompt-compiler",
            logical_key=f"{target['logical_key']}:compiled-prompt", asset_role="compiled_image_prompt",
            name="已编译视觉提示词", content=compiled.positive_prompt, entity_ids=list(entity_ids),
            parent_asset_ids=[contract_asset["asset_id"]], contract_artifact_id=contract_asset["asset_id"],
            metadata={"visual_context": context, "prompt_compiler": "v3-visual-v1", "source_prompt_asset_id": source_id},
        )
        ...
        return {**payload, "prompt_asset_id": prompt_asset["asset_id"], "params": params,
                "generation_contract_id": contract_asset["asset_id"], "visual_context": context,
                "visual_direction": direction, "character_appearances": selections}

...
本规则不新增阶段或交付物，只提高现有剧本/交接内容的结构质量。严格保持用户原故事的事实与因果，不补造未提供的剧情结果。
...
不要把镜头机位、景别、摄影参数写进稳定身份；这些属于④分镜。①剧本只提供剧情事实和动作依据，不提前写最终 image_prompt / 图片提示词。
...
本规则参考成熟影视资产库“角色可拥有可修改形象版本、渲染候选再选择正式版本”的做法，但继续使用小段映画现有角色阶段、候选和版本系统。
...
本规则把现有视觉阶段从“泛化美术描述”收紧为可复用视觉资产设计。项目整体创意方向与单个资产身份必须分开。
